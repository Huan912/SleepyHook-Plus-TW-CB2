...
const char* CRegistry::ReadString(const char* key, const char* defaultValue)
{
	...
	lResult = RegQueryValueExA(m_hKey, key, 0, &dwType, (unsigned char*)value, &dwSize);
...
void CRegistry::Init(void)
{
	...
	sprintf_s(szModelKey, "Software\\Nexon\\%s\\Settings\\", GetPlatformName());
	lResult = RegCreateKeyExA(HKEY_CURRENT_USER, szModelKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &m_hKey, &dwDisposition);
